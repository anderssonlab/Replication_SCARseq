...
## ./RFD_smooth_find_breakpoints.sh ../Okazaki_data/Okazaki_mm10_r1_F.nodup.bw ../Okazaki_data/Okazaki_mm10_r1_R.nodup.bw 1000 20 20 results mm10.chrom.sizes
...
CPM=`cat ${OUT}/*_windows_F.tab ${OUT}/*_windows_R.tab | awk '{SUM += ($4+1)} END {print SUM/10^6}'`
...
for CHR in $( cut -f 1 $CHROM | sort | uniq ); do
	./RFD_smooth.pl ${OUT}/${CHR}_windows_F_CPM.tab ${OUT}/${CHR}_windows_R_CPM.tab $RADIUS $DRADIUS $ZRADIUS > ${OUT}/${CHR}_windows_RFD.txt &
done
wait

## Gather output
if [ -f ${OUT}/Okazaki_smooth_results_w${WIN}_s${RADIUS}_d${DRADIUS}_z${ZRADIUS}.txt ]; then rm ${OUT}/Okazaki_smooth_results_w${WIN}_s${RADIUS}_d${DRADIUS}_z${ZRADIUS}.txt; fi
touch ${OUT}/Okazaki_smooth_results_w${WIN}_s${RADIUS}_d${DRADIUS}_z${ZRADIUS}.txt
for CHR in $( cut -f 1 $CHROM | sort | uniq ); do
	paste ${OUT}/${CHR}_windows.bed ${OUT}/${CHR}_windows_F.tab ${OUT}/${CHR}_windows_R.tab ${OUT}/${CHR}_windows_F_CPM.tab ${OUT}/${CHR}_windows_R_CPM.tab ${OUT}/${CHR}_windows_RFD.txt | awk '$8>0 || $14>0' | cut -f -3,8,14,20,26,30- >> ${OUT}/Okazaki_smooth_results_w${WIN}_s${RADIUS}_d${DRADIUS}_z${ZRADIUS}.txt
done
